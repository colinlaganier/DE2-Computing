...
# NOTE: it is recommended to keep density below 0.8
target, perfect_solution = utils.generate_target(
    width=1000, height=1000, density=0.5, forbidden_pieces=the_forbidden_pieces)

solution = Tetris(deepcopy(target))
valid, missing, excess, error_pieces = utils.check_solution(
    target, solution, the_forbidden_pieces)  # checks if the solution is valid
...
if not valid or len(error_pieces) != 0:
    ...
    time_set = timeit.timeit('Tetris({})'.format(
    ...
    if time_set > 600:
        ...
        print("Accuracy: {:.2f}%\n".format(
            100-(100 * missing / total_blocks)-(100 * excess / total_blocks)))
